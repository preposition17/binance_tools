...
class AutoBuy:
    ...
    def get_proxy_list():
        ...
        logging.info(f'{get_time()}: Proxy list getting.')
    ...
    def make_session(self):
        ...
        logging.info(f'{get_time()}: Requests session making.')
    ...
    def get_session(self):
        ...
        logging.info(f'{get_time()}: Requests session getting.')
        if self.__session:
    ...
    def auth(self):
        ...
        logging.info(f'{get_time()}: Authentication.')
    ...
    def get_sale_start_time(self):
        ...
        logging.info(f'{get_time()}: Sale start time getting.')
        _session = self.get_session()
    ...
    def purchase(self):
        ...
        logging.info(f'{get_time()}: Waiting to sale...')
        while True:
    ...
    def purchase_trying(self):
        ...
        logging.info(f'{get_time()}: Starting purchase with {len(proxy_list)} processes.')
    ...
    def purchase_loop(self, _proxy, _session):
        ...
        logging.info(f'{get_time()}: Starting purchasing loop on process #{os.getpid()}.')
        while True:
            try:
                logging.info(f'{get_time()}: Trying purchase on process #{os.getpid()}.')
                self.purchase_request(_session, _proxy=_proxy)
                break
            except Exception as ex:
                logging.error(f'{get_time()}: Unsuccessful purchase on process #{os.getpid()}.'
                              f'\n{traceback.format_exc()}')
                print(f'{get_time()}: Error.')
                print(ex)
                continue
            except KeyboardInterrupt:
                logging.info(f'{get_time()}: Exiting on process #{os.getpid()}.')
                exit()
    ...
    def purchase_request(self, _session, _proxy: str):
        ...
        try:
            ...
            logging.info(f'{get_time()}: {_response.json()["message"]}, {_response.json()["success"]} on process #{os.getpid()}.')
            print(f'{get_time()}: {_response.json()["message"]}, {_response.json()["success"]}')
        except Exception as ex:
            logging.error(f'{get_time()}: Error on process #{os.getpid()}.'
                          f'\n{traceback.format_exc()}')
